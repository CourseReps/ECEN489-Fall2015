    ...
    ui(new Ui::MainWindow)
{
    ...
    connect(mThread, SIGNAL(setParameterValues(double,double,double,double,double)), this, SLOT(realtimeDataSlot(double,double,double,double,double)));
...
void MainWindow::realtimeDataSlot(double IRRange1, double IRRange2, double PumpRate1, double PumpRate2, double FlowRate)
{
    ...
    if(readytoput)
    {
        if(!stablestate1)
        {
            ...
            //TODO:
            compute_volumn();
            ui->textBrowser->setText(estimate_volumn);
        }
        else if (estimate_state)
        {
            estimate_state = flase;
