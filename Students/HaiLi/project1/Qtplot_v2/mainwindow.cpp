...
void MainWindow::realtimeDataSlot()
{
     ...
     if (key-lastPointKey >= 1) // at most add point every 10 ms
     {
         ...
         qy->prepare("select distinct DeviceID from TBL1");
         qy->exec();
         QString temper;
         int q=0;
         int pj=0;
         ...
         if (!(qry[i]->prepare("select FlowRate, PumpRate, IR_range, SolenoidState from TBL1 where DeviceID="+IDlist[i]+";")))
         {
             qDebug() << "Query failed:";
		 ...
		 Solenoid[i]=qry[i]->value(3).toString();}
    ...
    while(j<idcount){
    // add data to lines:
    //ui->textBrowser->setText(IDlist[j]);
    swt[j]=IDlist[j]+":"+Solenoid[j];
