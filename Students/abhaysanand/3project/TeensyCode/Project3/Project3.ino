...
//#define TANK2  //uncomment to enable PID control for tank 2
...
const String send_data1 = "send_data";
const String send_data2 = "send_data2";
unsigned long timestamp, last_timestamp;
short unsigned int state;
unsigned long state0_counts = 0;
unsigned long state1_counts = 0;
double sigmaQ;
double objectVolume;
double deltaLevel;
double heightdiff;
...
PID myPID(&Input, &Output, &Setpoint,20,0.55,0.5, DIRECT);
...
void setup()
{
    ...
    myPID.SetOutputLimits((double)20,(double)90);
}
...
void loop()
{
    ...
    //double pumpControl = map(Output,0,100,20,90);
    analogWrite(PumpPWM,Output);
    ...
    if(a%100 == 0)  //decreased from 500 to 100 to improve counter performance for volume calculation
    {
        ...
        case 0:
            if(a%500 == 0)
                Serial.println("State: Check 0");
            if(deltaLevel > THRES)      //still unstable, maintain state 0
                break;
            else if(state0_counts==100)                       //error is within threshold, begin wait for PID steady state (state 1)
            {
                state = 1;
                state0_counts = 0;
                state1_counts = 0;
        ...
        case 1:
            if(a%500 == 0)
              Serial.println("State: Check 1");
            if(deltaLevel < THRES)      //still within threshold
            {
                if(state1_counts == 100)   //accumulated enough time in wait state, acceptable PID steady state found, go to state 2
                {
                    state = 2;
                    break;
                }
        ...
        case 2:
            ...
            if(deltaLevel < THRES)      //hold here as long as still within threshold
            {
                if(a%500 == 0)
                  Serial.println("Object can be dropped");
                break;
            }
        ...
        case 3:
            if(a%500 == 0)
                Serial.println("State: Calculate");
            if(deltaLevel > THRES)      //sum up Qout until we're back inside the threshold
            {
                Serial.println("Flow calculation");
            ...
            else                        //we've fallen back within the threshold, stop summing and do the volume calculation
            {
                ...
                objectVolume = (objectVolume*1000) - 35;
    ...
    if(BTSerial.available())
    {
        ...
        if(0 == int(temp))
        {
            ...
            {
                String data1 = String(String(Input) + "," + String(flow_sense) + "," + String(Qout) + ",");
                double vol = 0;
                if(volFlag)
                {
                    vol = objectVolume;
            ...
            else if(send_data1 == BTget.substring(0,9))
            {
