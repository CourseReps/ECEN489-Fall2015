...
writetodatabase::writetodatabase(QObject *parent)
    :   QObject(parent)
{
...
void writetodatabase::startDeviceDiscovery(const QBluetoothAddress &localAdapter)
{
...
void writetodatabase::serviceDiscovered(const QBluetoothServiceInfo &serviceInfo)
{
    ...
    if(m_service.device().name() == "linvor"){
        ...
        btcommunication *teensy = new btcommunication(this);
...
void writetodatabase::connected(const QString &per)//const QString &per
{
...
void writetodatabase::disConnected()//const QString &per
{
    emit  finished();
}
...
void writetodatabase::initializedb(){

        QSqlDatabase db = QSqlDatabase::addDatabase("QSQLITE");
       // open the specified database
        db.setDatabaseName( "/home/uday/Desktop/Project2/data.db");
...
void writetodatabase::addtodb(const QString &data){
     ...
     if(rec_data[dt_ind] != "s")  dt_ind++;
          ...
          // open the specified database
           ...
           if(!db.open()){
                ...
                QString stmt1 =QString("INSERT INTO DATA VALUES('team5',' "+ QString("%1").arg(timestamp)+ " ',' " + rec_data[0] + " ',' " +  rec_data[1]+ " ',' " +
...
void writetodatabase::sendSetPoint(const QString setpoint){
