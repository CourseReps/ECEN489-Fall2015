...
void btThread::run()
{
    ...
    while(1)
    {
        ...
        if (NULL != btRead)
        {
            ...
            static double objVolume = 0;
            double temp = btList[3].toDouble();
            ...
            if(objVolume != 0)
            {
                label = "Connected to BT device | Object Volume = " + QString::number(objVolume);
            }
            else
            {
            ...
            emit setParameterValues(btRead);
        }
        else
        {
